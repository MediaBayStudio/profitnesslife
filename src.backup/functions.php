...
add_filter('acf/load_value/name=go', function ( $value, $post_id, $field ) {
  if ( $_GET['post'] && $_GET['action'] === 'edit' || $_GET['post_type'] === 'dish' ) {
    echo '<div class="acf-field create-images-block" style="width:50%;display:flex;align-items:flex-end" data-width="50"><button type="button" id="create-images" class="button button-small" onclick="createRecipe()" style="min-height:30px">Создать рецепт</button><span class="spinner"></span><span class="success hidden" aria-hidden="true" style="text-align:left;color:#008a20">Создано!</span></div>';
  }
  return $value;
}, 10, 3);
